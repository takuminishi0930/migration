...
classified_df=test_predictions[["テキスト","URL"]][test_predictions["Label"]==1].reset_index()
...
cell_list = worksheet.range("A1:B{}".format(len(classified_df)))
for i in range(len(classified_df)):
    cell_list[2*i].value = classified_df["テキスト"][i]
    cell_list[2*i+1].value = classified_df["URL"][i]
